...
int main(int argc, char* argv[])
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 600, "World War Battleship", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    LoadTextureImage("../../data/tc-earth_daymap_surface1.jpg");      // TextureImage0
    LoadTextureImage("../../data/tc-earth_nightmap_citylights.gif"); // TextureImage1
    LoadTextureImage("../../data/mar.jpg"); // TextureImage2
    LoadTextureImage("../../data/mare.jpg"); // TextureImage3
    LoadTextureImage("../../data/marselect.jpg"); // TextureImage4
    LoadTextureImage("../../data/mareseen.jpg"); // TextureImage5
    LoadTextureImage("../../data/mareseene.jpg"); // TextureImage6
    LoadTextureImage("../../data/mareseenselect.jpg"); // TextureImage7
        ...
        while (!glfwWindowShouldClose(window))
    {
        ...
        if(false)
        {
            ...
            camera_position_c  = (glm::vec4(x,3,z,1.0f)-glm::vec4(0.0,0.0,0.0,1.0f)) + camera_lookat_l;
            camera_position_c.w= 1.0;
            camera_view_vector = camera_lookat_l-camera_position_c; // Vetor "view", sentido para onde a câmera está virada
            camera_lookat_l.w = 0;
            camera_view_vector.w = 0;
...
#define SEEN 10
        ...
        model = model * Matrix_Translate(10,0,10) * Matrix_Scale(rsphare,rsphare,rsphare)*Matrix_Rotate_Y(glfwGetTime()*0.1);
        ...
        for(int i = 0; i<10; i++)
        {
            for(int j = 0; j<10; j++)
            {
                ...
                {
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(Chao[i][j].model));
                    glUniform1i(object_id_uniform, SEEN);
                    DrawVirtualObject("plane");
                }
            }
        }
        ...
        if(gamestart && missil_on)
        {
            ...
            if(t<=1 && glfwGetTime()-tempo>=0.01 && desenhado && kkk)
            {
                ...
                modelball =Matrix_Translate(pontosf.x,pontosf.y,pontosf.z) * Matrix_Scale(0.5,0.5,0.5);
                ...
                glm::vec4 mbbox_min = modelball * b_aux_bbox_mix;
                glm::vec4 mbbox_max = modelball * b_aux_bbox_max;
                glm::vec4 bbbox_min;
                glm::vec4 bbbox_max;
                ...
                glm::vec4 m_aux_bbox_mix;
                glm::vec4 m_aux_bbox_max;

                glm::vec4 p_aux_bbox_mix;
                glm::vec4 p_aux_bbox_max;
                for(int j = 0; j<2; j++)
                {
                    for(int i=0; i< nbarco; i++)
                    {
                ...
                p_aux_bbox_max = glm::vec4(g_VirtualScene["plane"].bbox_max.x,g_VirtualScene["plane"].bbox_max.y, g_VirtualScene["plane"].bbox_max.z, 1);
                p_aux_bbox_mix = glm::vec4(g_VirtualScene["plane"].bbox_min.x,g_VirtualScene["plane"].bbox_min.y, g_VirtualScene["plane"].bbox_min.z, 1);

                pbbox_max = chaoAtual.model * p_aux_bbox_max;
                pbbox_min = chaoAtual.model * p_aux_bbox_mix;
        ...
        for(int i = 0; i<2; i++)
        {
            ...
            {
                if(barco[i][j].alido && !barco[i][j].alive)
                {
